...
- Renaming a domain concept (e.g. "Monthly Goal" → "Sprint") requires deleting the legacy endpoint, hook, and validation schema in the same change — leaving the old read path mounted as a "fallback" silently wins for any consumer that wasn't migrated, and the bug surfaces as silent misrender (empty milestones, "No goal set", broken vision reminders) instead of a hard error. Define-of-done for any rename: zero grep hits for the old name in client/ and server/.
- Setup/onboarding gates belong in the route layer (e.g. `AccessGatedRoute`), not inside individual pages. A page-level `useEffect(() => setLocation("/setup"))` only fires when the user lands on that specific page — every other page becomes a deep-link bypass for incomplete onboarding. The route guard pattern needs a location-aware bypass on `/setup` itself to avoid redirect loops.
- Sprint reads must use `useActiveSprint` (returns the active sprint or null), never `useMonthlyGoal(YYYY-MM)`. The legacy hook was deleted in the read-path unification — adding a new YYYY-MM-keyed lookup will not find sprint rows because they're keyed by full startDate.
- Setup-wizard golden-path e2e (wizard → today, no mocks for the redirect path) is mandatory whenever onboarding state shape changes. The original incident shipped because every test in the wizard spec mocked all endpoints and never navigated to `/today`.

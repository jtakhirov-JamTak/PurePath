...
- **Key Features**:
  ...
  - **Habit Tracking**: Recurring habits with day-of-week cadence, duration, 5 categories (Health=green/emerald, Wealth=yellow, Relationships=red/rose, Self-Development=blue, Happiness=white/slate), and daily tracking. Habits can be typed (goal/learning/maintenance). **Binary flag** (`is_binary` column, default false): Binary items (e.g., "take medication") have 2 levels: 0=Not Done, 1=Done (green). Non-binary items have 3 levels: 0=Not Done (red), 1=Minimum (yellow), 2=Full (green). Binary toggle available in habit dialog and Eisenhower entry creation (manual add, edit, brain dump wizard). Progress metrics score binary level=1 as full completion. `habit_completions` table has `completion_level` (int 0-2) and `skip_reason` (varchar) columns. Same system applies to Eisenhower Q1/Q2 items. Each habit requires a "Motivating Reason" field.

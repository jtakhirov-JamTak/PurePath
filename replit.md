...
- **Key Features**:
  ...
  - **Habit Tracking**: Recurring habits with day-of-week cadence, duration, 5 categories (Health=green/emerald, Wealth=yellow, Relationships=red/rose, Self-Development=blue, Happiness=white/slate), and daily tracking. Habits can be typed (goal/learning/maintenance). 3-level completion tracking: 0=Not done (red), 1=Minimum 2-5min (yellow), 2=Full version (green). Cycling: blank → full(green) → minimum(yellow) → skipped(red, with skip reason dialog) → blank. Skip reasons: 10 predefined options (Low Capacity, High System Load, Avoidance, etc.). `habit_completions` table has `completion_level` (int 0-2) and `skip_reason` (varchar) columns. Same 3-level system applies to Eisenhower Q1/Q2 items. Each habit requires a "Motivating Reason" field.
